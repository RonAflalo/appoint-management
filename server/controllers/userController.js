...
const getWorkers = (req, res) => {
  ...
  const workers = db.prepare(`
    SELECT id, name
    FROM users
    WHERE role = 'worker' AND business_id = 1 AND is_active = 1
    ORDER BY name
  `).all();
...
const getSlots = (req, res) => {
  ...
  if (workerId) {
    const slots = getAvailableSlots({ workerId: Number(workerId), serviceId: Number(serviceId), date, businessId: 1 });
    return res.json({ success: true, slots });
  }

  // No specific worker - get all active workers and merge slots
  const workers = db.prepare(`
    SELECT id FROM users WHERE role = 'worker' AND business_id = 1 AND is_active = 1
  `).all();

  const allSlots = {};
  for (const worker of workers) {
    const workerSlots = getAvailableSlots({ workerId: worker.id, serviceId: Number(serviceId), date, businessId: 1 });
...
const getAvailableDays = (req, res) => {
  ...
  const workers = workerId
    ? [{ id: Number(workerId) }]
    : db.prepare("SELECT id FROM users WHERE role = 'worker' AND business_id = 1 AND is_active = 1").all();
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    let hasSlots = false;
    for (const worker of workers) {
      const slots = getAvailableSlots({ workerId: worker.id, serviceId: Number(serviceId), date: dateStr, businessId: 1 });
...
const bookAppointment = (req, res) => {
  ...
  const worker = db.prepare("SELECT id FROM users WHERE id = ? AND role = 'worker' AND is_active = 1").get(workerId);
  ...
  const result = db.prepare(`
    ...
    VALUES (1, ?, ?, ?, ?, ?, 'pending', ?)
  `).run(req.user.id, workerId, serviceId, startTimeFormatted, endTime, notes || null);

  const appointment = db.prepare(`
    SELECT a.*, s.name AS service_name, w.name AS worker_name
    FROM appointments a
...
module.exports = { getServices, getWorkers, getSlots, getAvailableDays, bookAppointment, getMyAppointments, cancelAppointment };
