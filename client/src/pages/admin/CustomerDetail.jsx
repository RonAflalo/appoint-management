...
const STATUS_LABELS = {
  ...
  reschedule_requested: { label: 'בקשת שינוי', color: 'bg-purple-100 text-purple-800' },
};
...
export default function AdminCustomerDetail() {
  ...
  if (!customer) return <div className="text-center text-gray-400 mt-20">לקוח לא נמצא</div>;
    ...
    <div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <div className="flex gap-4">
            <div className="text-center bg-indigo-50 rounded-lg px-4 py-3">
              <p className="text-2xl font-bold text-indigo-700">{appointments.length}</p>
              <p className="text-xs text-indigo-500">תורים</p>
            </div>
            <div className="text-center bg-green-50 rounded-lg px-4 py-3">
              ...
              <p className="text-xs text-green-500">סה"כ הכנסה</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <h2 className="text-lg font-semibold text-gray-800 mb-3">היסטוריית תורים</h2>
          {appointments.length === 0 ? (
            <div className="bg-white rounded-xl border border-gray-100 p-8 text-center text-gray-400">אין תורים עדיין</div>
          ) : (
            <div className="space-y-3">
              {appointments.map(appt => {
                const status = STATUS_LABELS[appt.status] || { label: appt.status, color: 'bg-gray-100 text-gray-700' };
                return (
                  <div key={appt.id} className="bg-white rounded-xl border border-gray-100 p-4">
                    <div className="flex items-start justify-between gap-2 flex-wrap">
                      <div>
                        ...
                        <p className="text-sm text-gray-500">עם {appt.worker_name}</p>
                        <p className="text-sm text-gray-500">{formatDateTime(appt.start_time)}</p>
                        {appt.notes && <p className="text-xs text-gray-400 mt-1">הערת לקוח: {appt.notes}</p>}
                      </div>
        ...
        {/* Admin notes */}
        <div>
          <h2 className="text-lg font-semibold text-gray-800 mb-3">הערות פנימיות</h2>
          <div className="bg-white rounded-xl border border-gray-100 p-4">
            <p className="text-xs text-gray-400 mb-2">הערות אלו גלויות רק לך ולא ללקוח</p>
            <textarea
              value={notes}
            ...
            <button
              ...
              className="mt-2 w-full bg-indigo-600 text-white text-sm py-2 rounded-lg hover:bg-indigo-700 disabled:opacity-50"
            >
